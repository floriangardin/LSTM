...
train :
  ...
  def __init__(self, p, inputs, s, costs, h=None, ha=None,model=None,teX=None,teY=None):
  ...
  ha: Theano variable or None
    ...
    # Store the model to get test results live :
    self.model = model
    self.teX = teX
    self.teY = teY
    ...
    # costs is [loss,error]
    # g is the gradient of the costs
    # input is [model.x, model.y]
    g = T.grad(costs[0], p)
  ...
  save_progress: string or None
    ...
    try:
      for u in xrange(first_iteration, 1 + num_updates):
        ...
        # for j in self.p:
        #     print j
        #     print numpy.max(numpy.abs(j.get_value()))
        #     print numpy.mean(j.get_value())
        #     print numpy.mean(numpy.abs(j.get_value()))
        #     print numpy.std(j.get_value())

        print 'update %i/%i,' % (u, num_updates),
        sys.stdout.flush()
        gradient = numpy.zeros(sum(self.sizes), dtype=theano.config.floatX)
        costs = []
        # An epoch corresponds to one gradient update for number_batch example
        # If we reach the end of the dataset we start from the beginning and we shuffle the dataset
        ...
        # Print the max value for each part of the gradient
        # print "Gradient debug"
        # for i in range(len(self.p)):
        #     print self.p[i]
        #     print numpy.max(result[i])
        #     print numpy.min(result[i])
        #     print numpy.mean(result[i])
        #     print numpy.mean(numpy.abs(result[i]))
        print 'cost=', numpy.mean(costs, axis=0),
        print 'lambda=%.5f,' % self.lambda_,
        print 'mu=%.5f,' % self.mu,
        sys.stdout.flush()
        # HERE WE PERFORM THE HESSIAN FREE OPTIMIZATION , we return the new cost , the delta we optimized and the backtracking step:
        ...
        # Predict testing set result :
        prediction = []
        pred_list = []
        good_example = 0
        total_lenght = 0
        labels = []
        for idx,i in enumerate(self.teX):

            # Do the prediction for each frame
            prediction.append(list(self.model.predict(i)))
            pred_list.append(argmax_index([prediction.count(j) for j in range(7)]))
            labels.append(self.teY[idx][0])
            good_example+=numpy.sum(numpy.asarray(prediction[-1])==self.teY[idx])
            total_lenght+=len(prediction[-1])
            # Check accuracy :

        print " Testing set accuracy : "
        accuracy_test.append(good_example/float(total_lenght))
        print accuracy_test[-1]
        print " File accuracy : "
        print "Accuracy : " + str(numpy.mean(numpy.asarray(labels) == numpy.asarray(pred_list)))
        ...
        # Append all the plot variables
        # W_plot.append(self.p[0].get_value())
        # W_i_plot.append(self.p[1].get_value())
        # W_out_plot.append(self.p[2].get_value())
        # h0_plot.append(self.p[3].get_value())
        # bh_plot.append(self.p[4].get_value())
        # by_plot.append(self.p[5].get_value())
        # if u%20==0:
        #     pause()
        #pause()
